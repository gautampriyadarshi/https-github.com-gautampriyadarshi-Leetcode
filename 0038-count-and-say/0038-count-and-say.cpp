...
public:
    string countAndSay(int n) 
    {
        ...
        for(int i = 3; i <= n ; i++)
        {
            ...
            string temp = "";
            str += "&"; // add a delimeter at the end
            int cnt = 1; // counter 
            
            // now loop from 1st value to last value
            for(int j = 1; j < str.length(); j++)
            {
                // this condition should be satisfied, if not that means new number 
                // has started occurring
                if(str[j] != str[j-1])
                {
                    temp += to_string(cnt); // add the counter to temp
                    temp += str[j-1]; // add the data of the counter
